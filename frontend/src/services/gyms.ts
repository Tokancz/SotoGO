...
export const gymsApi = {
  ...
  getState: (stopId: string) =>
    api.get<GymState>(`/me/gyms/${stopId}`).then((r) => r.data),

  /** Place one of your vehicles to defend an open gym. */
  deploy: (stopId: string, vehicleId: string) =>
    api.post<GymState>(`/me/gyms/${stopId}/deploy`, { vehicleId }).then((r) => r.data),

  /** Pull your defender out of a gym you hold. */
  recall: (stopId: string) =>
    api.post<GymState>(`/me/gyms/${stopId}/recall`).then((r) => r.data),
